...
import SidebarNode from '@/components/rubick/SideBarNode.vue';
...
type SimpleBarInstance = {
    recalculate: () => void;
    unMount: () => void;
    getScrollElement: () => HTMLElement;
};

type SimpleBarConstructor = new (element: HTMLElement) => SimpleBarInstance;
let simpleBar: SimpleBarInstance | null = null;

// IMPORTANT: clone Set to trigger Vue reactivity
...
function getSimpleBarConstructor(): SimpleBarConstructor | null {
    return (window as Window & { SimpleBar?: SimpleBarConstructor }).SimpleBar ?? null;
}

function scrollToActiveItem() {
    const activeItem = scrollContainer.value?.querySelector<HTMLElement>('.side-menu__link--active');
    if (!activeItem || !simpleBar) return;

    simpleBar.getScrollElement().scrollTop = activeItem.getBoundingClientRect().top - 250;
}

function initSimpleBar() {
    if (!scrollContainer.value) return;

    const SimpleBar = getSimpleBarConstructor();
    if (!SimpleBar) return;
    ...
    simpleBar = new SimpleBar(scrollContainer.value);
    scrollToActiveItem();
}
...
<template>
    <div class="side-menu group after:bg-noise fixed top-0 left-0 z-50 -ml-[275px] text-background transition-[margin] duration-200 before:fixed before:inset-0 before:hidden before:bg-black/80 before:backdrop-blur after:absolute after:inset-0 after:bg-primary xl:ml-0 before:xl:hidden after:xl:hidden dark:text-foreground dark:before:bg-foreground/5 dark:after:bg-background [&.side-menu--mobile-menu-open]:ml-0 [&.side-menu--mobile-menu-open]:before:block"
        ...
        <div class="close-mobile-menu fixed z-50 ml-[275px] hidden cursor-pointer xl:hidden [&.close-mobile-menu--mobile-menu-open]:block"
            :class="{ 'close-mobile-menu--mobile-menu-open': mobileOpen }" @click="emit('close-mobile')">
        ...
        <div
            ...
            <div
                ...
                <a class="flex items-center transition-[margin] duration-200 xl:ml-2 group-[.side-menu--collapsed]:xl:ml-6 group-[.side-menu--collapsed.side-menu--on-hover]:xl:ml-2"
                    href="#">
                ...
                <a href="#"
                    ...
                    <ChevronRight v-if="props.collapsed"
            ...
            <!-- Scrollable area -->
            <div ref="scrollContainer" class="simplebar z-20 h-full w-full overflow-x-hidden overflow-y-auto scroll-smooth px-4 pb-3
                ...
                <ul class="scrollable">
                    <template v-for="(item, index) in items" :key="index">
                        <!-- Group label -->
                        <li v-if="item.type === 'group'" class="side-menu__group-label">
                            {{ item.label }}
                        </li>

                        <!-- Menu item -->
                        <SidebarNode v-else :item="item" :item-key="String(index)" :is-active="isActive"
                            :is-open="isOpen" :toggle="toggle" :collapsed="collapsed" :is-hovering="isHovering"
            ...
            <div
                ...
                <div
                    ...
                    <div
                        ...
                        <img class="absolute top-0 h-full w-full object-cover" :src="assets(`fakers/profile-11.jpg`)"
                            alt="Profile" />
                    </div>
                ...
                <div class="hidden group-hover/profile:block">
                    <div
                        ...
                        <div class="flex flex-col gap-0.5">
                            <a class="-mx-3 flex gap-2.5 rounded-lg px-4 py-1.5 hover:bg-foreground/5" href="#">
                                <Users
